...
static void Task_OakSpeech13(u8 taskId)
{
    ...
    if (gTasks[taskId].data[3] < 0x4000)
    {
        ...
        if (gTasks[taskId].data[3] == 32)
        {
            ...
            PlayCry1(SPECIES_HOUNDOOM_MEGA, 0);
        }
    }
}
...
static void CreateNidoranFSprite(u8 taskId)
{
    ...
    DecompressPicFromTable(&gMonFrontPicTable[SPECIES_HOUNDOOM_MEGA], OakSpeechNidoranFGetBuffer(0), SPECIES_HOUNDOOM_MEGA);
    LoadCompressedSpritePaletteUsingHeap(&gMonPaletteTable[SPECIES_HOUNDOOM_MEGA]);
    SetMultiuseSpriteTemplateToPokemon(SPECIES_HOUNDOOM_MEGA, 0);
