...
static bool8 TryGenerateWildMon(const struct WildPokemonInfo * info, u8 area, u8 flags)
{
    ...
    {
    case WILD_AREA_LAND:
        if (TryGetAbilityInfluencedWildMonIndex(info->wildPokemon, TYPE_STEEL, ABILITY_MAGNET_PULL, &slot, LAND_WILD_COUNT))
            break;
        if (TryGetAbilityInfluencedWildMonIndex(info->wildPokemon, TYPE_ELECTRIC, ABILITY_STATIC, &slot, LAND_WILD_COUNT))
            break;
		if (TryGetAbilityInfluencedWildMonIndex(info->wildPokemon, TYPE_ELECTRIC, ABILITY_LIGHTNING_ROD, &slot, LAND_WILD_COUNT))
            break;
	    if (TryGetAbilityInfluencedWildMonIndex(info->wildPokemon, TYPE_FIRE, ABILITY_FLASH_FIRE, &slot, LAND_WILD_COUNT))
            break;
		
        slot = ChooseWildMonIndex_Land();
        break;
    case WILD_AREA_LAND_NIGHT:
        if (TryGetAbilityInfluencedWildMonIndex(info->wildPokemon, TYPE_STEEL, ABILITY_MAGNET_PULL, &slot, LAND_NIGHT_WILD_COUNT))
            break;
        if (TryGetAbilityInfluencedWildMonIndex(info->wildPokemon, TYPE_ELECTRIC, ABILITY_STATIC, &slot, LAND_NIGHT_WILD_COUNT))
            break;
		if (TryGetAbilityInfluencedWildMonIndex(info->wildPokemon, TYPE_ELECTRIC, ABILITY_LIGHTNING_ROD, &slot, LAND_NIGHT_WILD_COUNT))
            break;
	    if (TryGetAbilityInfluencedWildMonIndex(info->wildPokemon, TYPE_FIRE, ABILITY_FLASH_FIRE, &slot, LAND_NIGHT_WILD_COUNT))
            break;
		
        slot = ChooseWildMonIndex_LandNight();
        break;
    case WILD_AREA_WATER:
        if (TryGetAbilityInfluencedWildMonIndex(info->wildPokemon, TYPE_STEEL, ABILITY_MAGNET_PULL, &slot, WATER_WILD_COUNT))
            break;
        if (TryGetAbilityInfluencedWildMonIndex(info->wildPokemon, TYPE_ELECTRIC, ABILITY_STATIC, &slot, WATER_WILD_COUNT))
            break;
		if (TryGetAbilityInfluencedWildMonIndex(info->wildPokemon, TYPE_ELECTRIC, ABILITY_LIGHTNING_ROD, &slot, WATER_WILD_COUNT))
            break;
	    if (TryGetAbilityInfluencedWildMonIndex(info->wildPokemon, TYPE_FIRE, ABILITY_FLASH_FIRE, &slot, WATER_WILD_COUNT))
            break;
    ...
    if (headerId != 0xFFFF)
    {
        if (ExtractMetatileAttribute(currMetatileAttrs, METATILE_ATTRIBUTE_ENCOUNTER_TYPE) == TILE_ENCOUNTER_LAND)
        {
            ...
            else
            {
                ...
                {
					if (TryGenerateWildMon(gWildMonHeaders[headerId].landMonsNightInfo, WILD_AREA_LAND_NIGHT, WILD_CHECK_REPEL) == TRUE)
						{ 
							StartWildBattle();
							return TRUE;
						}
