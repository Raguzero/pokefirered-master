...
//static u8 ShowSplitIcon(u32 split); //Physical/Special Split from BE // VER COMO ARREGLARLO
...
#define temp_dexNum     vars[2]
    ...
    switch (sortMode)
    {
        ...
        case SORT_ALPHABETICAL:
            ...
            break;
        case SORT_HEAVIEST:
            for (i = NELEMS(gPokedexOrder_Weight) - 1; i >= 0; i--)
            {
        ...
        case SORT_LIGHTEST:
            ...
            break;
        case SORT_TALLEST:
            for (i = NELEMS(gPokedexOrder_Height) - 1; i >= 0; i--)
            {
...
void LoadAreaScreen(u8 taskId)
{
    switch (gMain.state)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            //ShowPokedexAreaScreen(sPokedexListItem->species, &sPokedexView->unk64E); // VER COMO ARREGLARLO
            SetVBlankCallback(gUnknown_030060B4);
            sPokedexView->unk64E = 0;
            gMain.state = 0;
...
static void Task_DisplayNewMonData(u8 taskId)
{
    ...
    switch (gTasks[taskId].data[0])
    {
        ...
        case 2:
            ...
            NewMonDataTypeIconSpriteId0 = CreateSprite(&gSpriteTemplate_MoveTypes, 10, 10, 2);
            NewMonDataTypeIconSpriteId1 = CreateSprite(&gSpriteTemplate_MoveTypes, 10, 10, 2);
...
static void PrintMonInfo(u32 natNum, u32 value, u32 owned, u32 newEntry)
{
   ...
   // if (newEntry)
    ...
    if (value == 0)
        ...
        value = num;
    ConvertIntToDecimalStringN(StringCopy(str, gOtherText_UnkF9_08_Clear_01), value, STR_CONV_MODE_LEADING_ZEROS, 3);
...
bool16 HasAllMons(void)
{
    ...
    for (i = 0; i < NATIONAL_DEX_ZARUDE; i++)
    {
        if (!GetSetPokedexFlag(i + 1, FLAG_GET_CAUGHT))
            return FALSE;
    }
...
{
    sub_80C1270(gText_SelectorArrow2, 0x90, left * 16 + 9);
}
...
static bool8 CalculateMoves(void)
{
    ...
    for (i=0; i<TUTOR_MOVE_COUNT; i++)
    {
        ...
        {
            sStatsMovesTutor[numTutorMoves] = sTutorMoves[i];
            numTutorMoves++;
        }
    }
...
static void PrintMoveNameAndInfo(u8 taskId, bool8 toggle)
{
    ...
    if (selected < numEggMoves)
    {
        ...
        PrintInfoScreenTextSmall(gText_ThreeHyphens, moves_x + 113, moves_y + 9);
        item = ITEM_LUCKY_EGG;
    }
    ...
    gTasks[taskId].data[3] = AddItemIconObject(ITEM_TAG, ITEM_TAG, item);
    ...
    {
        //before --> SetTypeIconPosAndPal(NUMBER_OF_MON_TYPES + gContestMoves[move].contestCategory, moves_x + 50, moves_y+1, 1);
        SetTypeIconPosAndPal(NUMBER_OF_MON_TYPES, moves_x + 50, moves_y+1, 1);
...
static void PrintMonStats(u8 taskId, u32 num, u32 value, u32 owned, u32 newEntry) //HGSS_Ui
{
    ...
    case 254:
        ...
        break;
    default:
        PrintInfoScreenTextSmall(gText_ThreeHyphens, gender_x, gender_y);
        break;
    }
...
static void PrintMonStatsToggle(u8 taskId)
{
    ...
    if (gTasks[taskId].data[5] == 0)
    {
        ...
        if (gBaseStats[species].eggGroup1 == EGG_GROUP_UNDISCOVERED || gBaseStats[species].eggGroup2 == EGG_GROUP_UNDISCOVERED)
        {
            ...
            PrintInfoScreenTextSmall(gText_ThreeHyphens, 78, base_y + base_offset*base_i);
        }
        else
        {
            PrintInfoScreenTextSmall(gText_Stats_EggCycles, base_x, base_y + base_offset*base_i);
	...
	else
    {
        ...
        if (n_items)
        {
            if (item1 == item2) // Un solo item con 100% de salir
            {
                ...
                ico1 = AddItemIconObject(ITEM_TAG+1, ITEM_TAG+1, item1);
                gSprites[ico1].x2 = abilities_x + 127;
                gSprites[ico1].y2 = abilities_y + 19;
            }
            else if (item1 == ITEM_NONE) // Un solo item con 5% de salir
            {
                ...
                ico1 = AddItemIconObject(ITEM_TAG+1, ITEM_TAG+1, item2);
                gSprites[ico1].x2 = abilities_x + 127;
                gSprites[ico1].y2 = abilities_y + 19;
            }
            else // Hay un item con 50% de salir, y puede que otro
 {
                ...
                if (item2 != ITEM_NONE) // dos items, falta dibujar el de 5%
                {
                    ...
                    ico2 = AddItemIconObject(ITEM_TAG+2, ITEM_TAG+2, item2);
                    gSprites[ico2].x2 = abilities_x + 127;
                    gSprites[ico2].y2 = abilities_y + 19 + 30;
                }
            }
        }
