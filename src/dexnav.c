...
#define WINDOW_MOVE_NAME_X  (WINDOW_COL_1 + (GetFontAttribute(sDexNavSearchDataPtr->windowId, FONTATTR_MAX_LETTER_WIDTH)))
...
static void AddSearchWindowText(u16 species, u8 proximity, u8 searchLevel, bool8 hidden)
{
    ...
    AddTextPrinterParameterized3(sDexNavSearchDataPtr->windowId, 0, 85, 0, sSearchFontColor, TEXT_SPEED_FF, gStringVar4);
    ...
    while (topY < botY)
    {
        while (topX < botX)
        {
            ...
            case ENCOUNTER_TYPE_WATER:
                if (MetatileBehavior_IsSurfable(tileBehaviour)) // Before (MetatileBehavior_IsSurfableWaterOrUnderwater(tileBehaviour))
                {
...
static bool8 DexNav_InitBgs(void)
{
    ...
    ResetAllBgsCoordinatesAndBgCntRegs(); // BEFORE ResetAllBgsCoordinates();
...
static void PrintMapName(void)
{
    ...
    AddTextPrinterParameterized3(WINDOW_REGISTERED, 1, 60 + GetStringRightAlignXOffset(1, gStringVar3, 16 * GetFontAttribute(1, FONTATTR_MAX_LETTER_WIDTH)), 0, sFontColor_White, 0, gStringVar3);
